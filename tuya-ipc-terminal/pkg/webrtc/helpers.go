...
func UnmarshalMedias(descriptions []*sdp.MediaDescription) (medias []*utils.Media) {
	...
	for _, kind := range []string{utils.KindVideo, utils.KindAudio} {
		for _, md := range descriptions {
			if md.MediaName.Media != kind {
				continue
			}

			media := utils.UnmarshalMedia(md)
			switch media.Direction {
			case utils.DirectionSendRecv:
				media.Direction = utils.DirectionRecvonly
				medias = append(medias, media)

				media = media.Clone()
				media.Direction = utils.DirectionSendonly
...
func SkipNonMediaCodecs(input []*utils.Codec) (output []*utils.Codec) {
...
func WithResampling(medias []*utils.Media) []*utils.Media {
	for _, media := range medias {
		if media.Kind != utils.KindAudio || media.Direction != utils.DirectionSendonly {
			continue
		}

		var pcma, pcmu, pcm, pcml *utils.Codec

		for _, codec := range media.Codecs {
			switch codec.Name {
			case utils.CodecPCMA:
				if codec.ClockRate != 0 {
					pcma = codec
				} else {
					pcma = nil
				}
			case utils.CodecPCMU:
				if codec.ClockRate != 0 {
					pcmu = codec
				} else {
					pcmu = nil
...
func MimeType(codec *utils.Codec) string {
	switch codec.Name {
	case utils.CodecH264:
		return webrtc.MimeTypeH264
	case utils.CodecH265:
	...
	case utils.CodecOpus:
		return webrtc.MimeTypeOpus
	case utils.CodecG722:
